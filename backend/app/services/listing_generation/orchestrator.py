...
class ListingGenerationOrchestratorService:
    ...
    def generate(self, request: GenerateListingSectionRequest) -> ListingSectionResult:
        ...
        if request.section == "seo_title":
            out = self._generate_title(request, settings, rules, merged_banned)
        elif request.section == "bullet_points":
            out = self._generate_bullets(request, settings, rules, merged_banned)
        elif request.section == "description":
            out = self._generate_description(request, settings, rules, merged_banned)
        else:
            out = self._generate_keywords(request, settings, rules, merged_banned)
    ...
    def _generate_title(self, request, settings, rules: InjectedRules, merged_banned: list[str]) -> ListingSectionResult:
    ...
    def _generate_bullets(self, request, settings, rules: InjectedRules, merged_banned: list[str]) -> ListingSectionResult:
        rules_eff = rules.model_copy(update={"banned_phrases": merged_banned})
        system = build_bullets_system_prompt()
    ...
    def _generate_description(self, request, settings, rules: InjectedRules, merged_banned: list[str]) -> ListingSectionResult:
    ...
    def _generate_keywords(self, request, settings, rules: InjectedRules, merged_banned: list[str]) -> ListingSectionResult:
        max_b = rules.backend_search_terms_max_bytes or settings.listing_backend_search_terms_max_bytes
        rules_eff = rules.model_copy(update={"banned_phrases": merged_banned})
        system = build_keyword_strategy_system_prompt()
